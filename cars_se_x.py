...
trainset    = torchvision.datasets.ImageFolder(root='/data/xushuai/Aircraft/train', transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=32, shuffle=True, num_workers=16, drop_last = True)

testset = torchvision.datasets.ImageFolder(root='/data/xushuai/Aircraft/test', transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=32, shuffle=True, num_workers=16, drop_last = True)
...
class Spatial_SEModule(nn.Module):
    ...
    def forward(self, x):
        ...
        spatial_feature_grad=spatial_feature.register_hook(self.save_gradient)
...
class model_bn(nn.Module):
    def __init__(self, model, feature_size,classes_num):
        ...
        self.se_module = Spatial_SEModule(2048, reduction=16)


        self.num_ftrs = 2048
        self.max = nn.MaxPool2d(kernel_size=14)
    ...
    def forward(self, x):
        ...
        return x2
net =model_bn(net, 512, 100)

# net = VGG('VGG11')
#x2 = torch.randn(2,3,224,224)
#print(net(Variable(x2)).size())
...
if use_cuda:
    ...
    net.s_se_module= torch.nn.DataParallel(net.se_module)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        outputs= net(inputs)
        one_hot = torch.max(outputs, 1, keepdim=True, out=None)[0]
        sig=net.se_module.get_sig().squeeze()
        optimizer.zero_grad()
        one_hot.backward(torch.ones_like(one_hot),retain_graph=True) #反向传播之后，为了取得目标层梯度
        grad_val = net.se_module.get_gradients()[-1].data
        weights = torch.mean(grad_val,dim = (2, 3))
        weights=torch.nn.functional.softmax(weights,dim=1)
        #optimizer.zero_grad()
        #sig=net.se_module.get_sig().squeeze()
        kl_loss=torch.nn.functional.kl_div(weights.log(),sig, size_average=None, reduce=None, reduction='sum')
